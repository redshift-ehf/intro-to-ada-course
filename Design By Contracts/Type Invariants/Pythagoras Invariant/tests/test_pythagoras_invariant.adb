...
with Ada.Assertions;
with Ada_Check;
with Pythagoras_Invariant;

procedure Test_Pythagoras_Invariant is
   use Pythagoras_Invariant;
   ...
   begin
      ...
      begin
         return "built" & Length'Image (H_Of (T)) & Length'Image (C1_Of (T))
                & Length'Image (C2_Of (T));
      end;
   exception
      when Ada.Assertions.Assertion_Error => return "refused";
   end Outcome;
begin
   Ada_Check.Suite ("Pythagorean Theorem: Type Invariant");
