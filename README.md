...
Three chapters out of the nine watched for it came back with a `content:` naming everything except
the chapter just written; the new `section-info.yaml` was never touched at all, and nothing was
logged. The other six produced a byte-identical file and the new `section-info.yaml` was touched
immediately.
...
six successes — same steps, same order, both outcomes.
